...
def convert_swe_rl_to_verl_format(example, idx, split):
    ...
    data = {
        ...
        "ability": "swe_rl",
        "reward_model": {"style": "rule", "ground_truth": example["patch"]},
        "extra_info": {
            "split": split,
            "index": idx,
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--test_size", type=int, default=64, help="Number of samples for test set (default: 64)")
    ...
    processed_train_dataset = train_dataset.map(
        function=lambda example, idx: convert_swe_rl_to_verl_format(example, idx, "train"), with_indices=True
    )
    processed_test_dataset = test_dataset.map(
        function=lambda example, idx: convert_swe_rl_to_verl_format(example, idx, "test"), with_indices=True
    )
